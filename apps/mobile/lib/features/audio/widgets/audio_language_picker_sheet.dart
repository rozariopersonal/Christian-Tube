...
class AudioLanguagePickerSheet extends StatefulWidget {
  ...
  const AudioLanguagePickerSheet({
    super.key,
...
class _AudioLanguagePickerSheetState extends State<AudioLanguagePickerSheet> {
  ...
  int _calculateSelectedTracksCount() {
    if (_isAllSelected()) {
      return widget.trackCounts['All'] ?? 0;
    }
    int total = 0;
  ...
  String _selectionSubtitle() {
    if (_isAllSelected()) {
      final total = widget.trackCounts['All'] ?? 0;
      return 'All Languages ($total tracks)';
    }
    final count = _currentSelection.length;
    final tracks = _calculateSelectedTracksCount();
    return '$count ${count == 1 ? 'Language' : 'Languages'} selected • $tracks tracks';
  }
                  ...
                  Expanded(
                    child: FilledButton.icon(
                      ...
                      label: Text(
                        isAll
                            ? 'Apply (All Languages)'
                            : 'Apply (${_currentSelection.length} Languages • ${_calculateSelectedTracksCount()} Tracks)',
