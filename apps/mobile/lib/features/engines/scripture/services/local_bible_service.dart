...
class LocalBibleService {
  ...
  // Bible and scripture-reference payloads only ship inside the Christian App.
  // For every other instance (e.g. Centum Academy) they stay empty, so the
  // bundled assets are never seeded into that build.
  static bool get _includeBundled => AppConfig.instanceId == 'christian_tube';
  final Set<String> _webInstalledVersions = {};

  // Bibles bundled into the Christian App under assets/data (generated by
  // tool/prepare_assets.js, not committed). Each entry maps a catalog version
  // id to its bundled asset path.
  final Map<String, String> _bundledAssets = {
    'WEB': 'assets/data/bible_web.json',
    'KJV': 'assets/data/bible_kjv.json',
    'ASV': 'assets/data/bible_asv.json',
    'BBE': 'assets/data/bible_bbe.json',
  ...
  bool _bundledDataAvailable = false;
  bool _bundledInMemorySeeded = false;
  Future<void>? _bundledSeedFuture;

  /// True when this build owns bundled bible text for [versionId] (Christian
  /// App with the generated assets/data payload present).
  bool isBundledVersion(String versionId) =>
      _bundledDataAvailable && _bundledAssets.containsKey(versionId);

  Future<void> _checkBundledDataAvailable() async {
    try {
      await rootBundle.load('assets/data/scriptures.json');
      _bundledDataAvailable = _includeBundled;
    } catch (_) {
      _bundledDataAvailable = false;
    }
  }

  Future<void> initialize() async {
    _seedAllPolyglotVerses();
    await _checkBundledDataAvailable();

    if (kIsWeb) {
      if (!_bundledInMemorySeeded && _bundledDataAvailable) {
    ...
    // Seed the bundled bibles in the background so app startup stays fast.
    // The versions already count as installed (see hasVerses), and readers
    // wait on this future before querying a bundled version.
    if (_bundledDataAvailable) {
      _bundledSeedFuture = _seedBundledBibles();
    }
  }

  Future<void> _seedBundledBibles() async {
    if (kIsWeb || !_bundledDataAvailable) return;
    final db = _db;
    if (db == null) return;
  ...
  Future<bool> hasVerses(String versionId) async {
    if (kIsWeb) {
      if (_webVerses.keys.any((k) => k.startsWith('${versionId}_'))) {
        return true;
      }
      return isBundledVersion(versionId);
    }
    final db = _db;
    ...
    // Bundled versions are installed by definition even while the background
    // seed is still writing their text rows.
    return isBundledVersion(versionId);
  }
  ...
  Future<List<String>> getInstalledVersionIds() async {
    final bundled = _bundledDataAvailable ? _bundledAssets.keys : const <String>[];

    if (kIsWeb) {
      return <String>{..._webInstalledVersions, ...bundled}.toList();
    }
    ...
    final ids = results.map((r) => r['id'] as String).toSet()
      ..addAll(_webInstalledVersions)
      ..addAll(bundled);
    return ids.toList();
  }
  ...
  Future<List<Map<String, dynamic>>> getChapter(String versionId, String bookName, int chapter) async {
    ...
    // Bundled text is written by a background seed; wait for it so readers
    // always get the full chapter once it becomes available.
    if (isBundledVersion(versionId)) {
      final pending = _bundledSeedFuture;
