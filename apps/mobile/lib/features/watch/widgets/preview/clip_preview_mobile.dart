...
Widget buildPlatformClipPreview({
  ...
  return _MobileClipPreviewWidget(
    key: key,
    videoId: videoId,
...
class _MobileClipPreviewWidget extends StatefulWidget {
  ...
  const _MobileClipPreviewWidget({
    super.key,
  ...
  @override
  State<_MobileClipPreviewWidget> createState() => MobileClipPreviewWidgetState();
}

class MobileClipPreviewWidgetState extends State<_MobileClipPreviewWidget> {
    ...
    try {
      _controller = YoutubePlayerController(
        ...
        flags: YoutubePlayerFlags(
          ...
          enableCaption: false,
          startAt: widget.clipStartTime.toInt(),
        ),
      )..addListener(_onPlayerStateChanged);
    } catch (e) {
      debugPrint('Error initializing MobileClipPreview: $e');
    }
  }
  ...
  void _onPlayerStateChanged() {
    ...
    if (pos >= widget.clipEndTime || pos < widget.clipStartTime - 1.0) {
      if (widget.isLooping) {
  ...
  void seekTo(double seconds) {
    ...
    if (_controller != null) {
      _controller!.seekTo(
        Duration(milliseconds: (_currentPosition * 1000).toInt()),
        allowSeekAhead: true,
      );
      if (_isPlaying) {
        _controller!.play();
  ...
  void didUpdateWidget(covariant _MobileClipPreviewWidget oldWidget) {
    ...
    if (oldWidget.videoId != widget.videoId) {
      _controller?.load(widget.videoId, startAt: widget.clipStartTime.toInt());
    }
  }
