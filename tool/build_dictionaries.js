...
async function main() {
  ...
  // 2. Real Tamil-to-Tamil Dictionary (from PTTamil.db)
  let taEntries = [];
  const ptDbPath = path.join(__dirname, '..', 'data', 'dictionaries_raw', 'PTTamil.db');
  if (fs.existsSync(ptDbPath)) {
    console.log('Loading Real Tamil-to-Tamil PTTamil dataset...');
    const ptDb = new DatabaseSync(ptDbPath);
    const rows = ptDb.prepare('SELECT word, meanings FROM dictionary').all();
    for (const r of rows) {
      if (r.word && r.meanings) {
        taEntries.push([r.word.trim(), '', '', r.meanings.trim(), '']);
      }
    }
    ptDb.close();
    console.log(`Loaded ${rows.length} real Tamil entries from PTTamil.db.`);
  }

  // 2b. Core Tamil Bible & Spiritual Lexicon
  const BIBLE_TAMIL_TERMS = [
  ...
  for (const item of BIBLE_TAMIL_TERMS) {
    taEntries.push(item);
  }

  // 2c. True Tamil Dictionary (from Wiktionary TSV)
  ...
  // 2d. Tamil English Reverse/Forward from tamil.json
  const taRawPath = path.join(__dirname, '..', 'data', 'dictionaries_raw', 'tamil.json');
  if (fs.existsSync(taRawPath)) {
    console.log('Loading Tamil JSON dataset for English-to-Tamil lookups...');
